...
| Social content generation | Groq API — Llama 3.3 70B (fast, no subprocess) |
...
| Groq API (optional) | AI social content | [console.groq.com](https://console.groq.com) → create API key → set `GROQ_API_KEY` |
...
ODOO_USERNAME=admin
ODOO_PASSWORD=admin
GROQ_API_KEY=<optional — enables AI-generated social post copy via Llama 3.3 70B>
```

> **GROQ_API_KEY** is optional but recommended. When set, Facebook and Instagram post copy is generated by Groq's Llama 3.3 70B model — fast, topic-aware, and requires no Claude subprocess. Without it, high-quality template fallback copy is used.
...
| `gmail_watcher.py` | Polls Gmail every 10s (configurable via `GMAIL_CHECK_INTERVAL`) |
| `whatsapp_watcher.py` | Monitors WhatsApp Web |
| `filesystem_watcher.py` | Inbox → Needs_Action routing (every 5s) |
...
| `scheduler.py` | LinkedIn post daily 09:00 · CEO briefing Mon 08:00 · Log cleanup Mon 10:00 |

---

## Scheduled Jobs

`scheduler.py` runs three automatic jobs each week:

| Job | Schedule | What it does |
|-----|----------|-------------|
| LinkedIn post | Daily 09:00 | Generates a topic-aware post draft → `Pending_Approval/linkedin/` |
| CEO Briefing | Monday 08:00 | Pulls Odoo revenue + task stats → `Briefings/YYYY-MM-DD_Monday_Briefing.md` |
| Log cleanup | Monday 10:00 | Deletes audit logs older than 90 days (runs after briefing + LinkedIn) |

On startup, `scheduler.py` auto-catches missed jobs — if it's Monday after 08:00 and the briefing hasn't run yet, it runs immediately.
...
├── Screenshots/            ← Browser screenshots (viewable in dashboard)
├── Scrapes/                ← Browser scrape results (viewable & deletable in dashboard)
...
**Social posts using template copy instead of AI copy**
- Check that `GROQ_API_KEY` is set in `.env`
- Groq is optional — if the key is missing or the API fails, quality template fallback copy is used automatically
